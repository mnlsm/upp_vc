...
public:
    ...
    int    GetGlyph(wchar chr) {
        return cmap[HIBYTE(chr)][LOBYTE(chr)];
    }
    word   GetAdvanceWidth(wchar chr) {
        return glyphinfo[GetGlyph(chr)].advanceWidth;
    }
...
private:
    struct CharPos : Moveable<CharPos> {
        word fi, ci;
    };
    ...
    inline double Pt(double dot) {
        return 0.12 * dot;
    }

    int    Pos() {
        return offset.GetCount() + 1;
    }
    int    BeginObj();
    void   EndObj();
...
public:
    ...
    PdfDraw(int pagecx, int pagecy) {
        Init(pagecx, pagecy, 0);
    }
    PdfDraw(Size pgsz = Size(5100, 6600)) {
        Init(pgsz.cx, pgsz.cy, 0);
    }
};
