...
static bool StdDisplayErrorFn(const Value& e) {
    ...
    MessageBoxA(NULL, s, GetExeTitle(), MB_OK | MB_ICONQUESTION);
    return true;
}
...
Value  Ctrl::GetData() const {
    return Value();
}

void Ctrl::Paint(Draw& draw)                        {}
int  Ctrl::OverPaint() const {
    return 0;
}
...
void Ctrl::ChildGotFocus() {
    GuiLock __;
    if(parent_)
        parent_->ChildGotFocus();
}

void Ctrl::ChildLostFocus() {
    GuiLock __;
    if(parent_)
        parent_->ChildLostFocus();
}

void Ctrl::ChildAdded(Ctrl *q) {
    GuiLock __;
    if(parent_)
        parent_->ChildAdded(q);
}

void Ctrl::ChildRemoved(Ctrl *q) {
    GuiLock __;
    if(parent_)
        parent_->ChildRemoved(q);
}
...
//      if(!parent_) WndEnable(enabled);
        if(top_)
            WndEnable(enabled);
// 01/12/2007 - END
        if(!enabled && parent_ && HasFocusDeep())
...
bool Ctrl::IsShowEnabled() const {
    GuiLock __;
    return IsEnabled() && (!parent_ || parent_->IsShowEnabled());
}
...
#endif
    if(IsChild())
        s << "(parent_ " << String(typeid(*parent_).name()) << ")";
    else
...
void Ctrl::Dump(Stream& s) const {
    GuiLock __;
    LG(Name());
    LG(sFLAG(backpaint) << sFLAG(inframe_) << sFLAG(visible) << sFLAG(enabled) <<
...
Ctrl::Ctrl() {
    ...
    LLOG("Ctrl::Ctrl");
    destroying = false;
    parent_ = prev = next = firstchild = lastchild = NULL;
    top_ = NULL;
    exitcode = 0;
...
void Ctrl::DoRemove() {
    ...
    if(HasFocusDeep()) {
        ...
        if(destroying) {
            if(parent_) {
                LLOG("parent_ - deferred SetFocus / ChildLostFocus; parent_ = " << UPP::Name(parent_));
                defferedSetFocus = parent_;
                defferedChildLostFocus.Add(parent_);
            }else if(IsPopUp()) {
                LLOG("Remove Popup");
                Ctrl *owner = GetOwner();
            ...
            DoKillFocus(fc, NULL);
            if(parent_) {
                LLOG("DoRemove -> SetFocus(" << UPP::Name(parent_) << "), focusCtrl = " << UPP::Name(focusCtrl) << ", fc = " << UPP::Name(fc));
                bool b = IsWantFocus();
                NoWantFocus();
                parent_->SetFocus0(false);
                WantFocus(b);
            }else if(IsPopUp()) {
                LLOG("Remove Popup");
                Ctrl *owner = GetOwner();
...
Font StdFontZ(int height) {
    return FontZ(Font::STDFONT, height);
}
Font SansSerifZ(int height) {
    return FontZ(Font::SANSSERIF, height);
}
Font SerifZ(int height) {
    return FontZ(Font::SERIF, height);
}
Font MonospaceZ(int height) {
    return FontZ(Font::MONOSPACE, height);
}

Font ScreenSansZ(int height) {
    return FontZ(Font::SCREEN_SANS, height);
}
Font ScreenSerifZ(int height) {
    return FontZ(Font::SCREEN_SERIF, height);
}
Font ScreenFixedZ(int height) {
    return FontZ(Font::SCREEN_FIXED, height);
}
Font RomanZ(int height) {
    return FontZ(Font::ROMAN, height);
}
Font ArialZ(int height) {
    return FontZ(Font::ARIAL, height);
}
Font CourierZ(int height) {
    return FontZ(Font::COURIER, height);
}
...
bool Ctrl::ClickFocus() {
    return _ClickFocus;
}
void Ctrl::ClickFocus(bool cf) {
    _ClickFocus = cf;
}
