...
private:
    ...
    void            Begin(const char *cmd, int param) {
        Begin();
        Command(cmd, param);
    }
    void            End() {
    ...
    void            PutBinHex(const String& s) {
        PutBinHex(s, s.GetLength());
    }
    ...
    struct Group {
        Group(RTFEncoder *owner) : owner(owner) {
            owner->Begin();
        }
        Group(RTFEncoder *owner, const char *cmd) : owner(owner) {
            owner->Begin(cmd);
        }
        Group(RTFEncoder *owner, const char *cmd, int param) : owner(owner) {
            owner->Begin(cmd, param);
        }
        ~Group() {
            owner->End();
        }
    ...
    Command("deff", 0);
    {
        ...
        for(int i = 0; i < used_faces.GetCount(); i++) {
            ...
            if(fn == Font::SYMBOL) {
                ...
                Command("froman");
            else if(fn == Font::ARIAL || fn == Font::TAHOMA)
                Command("fswiss");
            if(!IsNull(fontcpg))
                Command("fcharset", cs);
            Space();
    ...
    for(int i = 0; i < txt.GetPartCount(); i++) {
        if(txt.IsTable(i))
            ...
            if(para.format.bullet == RichPara::BULLET_TEXT) {
                ...
                case RichPara::BULLET_ROUND:
                    sym_face = SYMBOL_INDEX;
                    sym_char = 0xB7;
                    break;
                case RichPara::BULLET_ROUNDWHITE:
                    sym_face = WINGDINGS_INDEX;
                    sym_char = 0xA1;
                    break;
