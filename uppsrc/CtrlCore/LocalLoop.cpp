...
RectTracker& RectTracker::Dashed() {
    pattern = RectTracker_dashed;
    return *this;
}
RectTracker& RectTracker::Solid() {
    pattern = RectTracker_solid;
    return *this;
}
...
void RectTracker::MouseMove(Point, dword) {
    ...
    if(tx == ALIGN_CENTER && ty == ALIGN_CENTER) {
        ...
        if(keepratio) {
            ...
            }else if(tx == ALIGN_RIGHT)
                rect.bottom = rect.top + cy;
            else if(ty == ALIGN_BOTTOM)
                rect.right = rect.left + cx;
        }
    }
    if(rect != o) {
