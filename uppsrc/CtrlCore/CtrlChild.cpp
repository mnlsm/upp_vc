...
void  Ctrl::AddChild(Ctrl *q, Ctrl *p) {
    ...
    if(q->parent_) {
        ...
        if(q->parent_ == this) {
            ...
            q->parent_->RemoveChild(q);
        }
    }
    q->parent_ = this;
    if(p) {
        ASSERT(p->parent_ == this);
        q->prev = p;
        q->next = p->next;
        if(p == lastchild)
            lastchild = q;
...
bool Ctrl::HasChild(Ctrl *q) const {
    GuiLock __;
    return q && q->IsChild() && q->parent_ == this;
}
...
const Ctrl *Ctrl::GetTopCtrl() const {
    return const_cast<Ctrl *>(this)->GetTopCtrl();
}

const Ctrl *Ctrl::GetOwner() const {
    return const_cast<Ctrl *>(this)->GetOwner();
}

Ctrl* Ctrl::GetTopCtrlOwner() {
    return GetTopCtrl()->GetOwner();
}

const Ctrl *Ctrl::GetTopCtrlOwner() const {
    return GetTopCtrl()->GetOwner();
}

Ctrl* Ctrl::GetOwnerCtrl() {
    GuiLock __;
    return !IsChild() && top_ ? top_->owner : NULL;
}

const Ctrl *Ctrl::GetOwnerCtrl() const {
    return const_cast<Ctrl *>(this)->GetOwnerCtrl();
}
