...
class DrawSurface : NoCopy {
    ...
    RGBA* Line(int i) const {
        ASSERT(i >= 0 && i < size.cy);
...
static tAlphaBlend fnAlphaBlend() {
    ...
    if(!inited) {
        inited = true;
        if(HMODULE hDLL = LoadLibraryA("msimg32.dll"))
...
Image Win32IconCursor(LPCSTR id, int iconsize, bool cursor) {
    HICON icon;
    if(cursor)
        icon = (HICON)LoadCursorA(0, id);
    else if(iconsize)
        icon = (HICON)LoadImageA(GetModuleHandle(NULL), id,
...
Image Win32DllIcon(const char *dll, int ii, bool large) {
    HICON icon;
    if(ExtractIconExA(dll, ii, large ? &icon : NULL, large ? NULL : &icon, 1) == 1)
...
Image Win32Icon(int id, int iconsize) {
    return Win32Icon(MAKEINTRESOURCEA(id), iconsize);
}
...
Image Win32Cursor(int id) {
    return Win32Cursor(MAKEINTRESOURCEA(id));
}
...
#define OLD_MAKEINTRESOURCE MAKEINTRESOURCE
...
#ifdef OLD_MAKEINTRESOURCE
#define MAKEINTRESOURCE OLD_MAKEINTRESOURCE
#undef OLD_MAKEINTRESOURCE
#endif
