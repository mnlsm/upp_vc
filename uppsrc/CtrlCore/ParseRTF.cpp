...
static int TwipDotsLim(int twips) {
    return minmax<int>(TwipDots(twips), 0, MAX_DOTS);
}
...
private:
    ...
    bool          PassText() {
        return PassIf(Token() == T_TEXT);
    }
    bool          PassGroup() {
        return PassIf(Token() == T_GROUP);
    }
    bool          PassEndGroup() {
        return PassIf(Token() == T_END_GROUP || token == T_EOF);
    }
    bool          PassEndGroup(int level);
    bool          PassCmd(const char *cmd) {
        return PassIf(Token() == T_COMMAND && command == cmd);
    }
    bool          PassQ(const char *cmd) {
        return PassIf(command == cmd);
    }

    void          SkipGroup() {
        SkipGroup(stack.GetCount());
    }
    ...
    void          ReadChar(word ch) {
        ReadText(WString(ch, 1));
    }
    void          ReadCommand();
...
//  bool          new_dest;
    ...
    struct TableState {
        TableState() : textcol(0), stylecol(0) {
            cells.Add();
        }
        ...
        while(!PassEndGroup()) {
            if(PassCmd("f"))
                ...
                case 136:
                    break; // Big5
                case 161:
                    n.charset = CHARSET_WIN1253;
                    break; // Greek
                case 162:
                    n.charset = CHARSET_WIN1254;
                    break; // Turkish
                case 163:
                    break; // Vietnamese
                case 177:
                    n.charset = CHARSET_WIN1255;
                    break; // Hebrew
                case 178:
                    break; // Arabic
                case 179:
                    break; // Arabic Traditional
                case 180:
                    break; // Arabic user
                case 181:
                    break; // Hebrew user
                case 186:
                    break; // Baltic
                case 204:
                    n.charset = CHARSET_WIN1251;
                    break; // Russian
                case 222:
                    break; // Thai
                case 238:
                    n.charset = CHARSET_WIN1250;
                    break; // Eastern European
                case 254:
                    break; // PC 437
                case 255:
                    n.charset = CHARSET_WIN1252;
...
void RTFParser::ReadPict() {
    ...
    while(!PassEndGroup())
        if(PassText())
            ...
            else if(PassQ("wmetafile")) {
                blip_type = WMF_BLIP;
                wmf_mode = command_arg;
    ...
    }else if(blip_type == DIB_BLIP || blip_type == PNG_BLIP || blip_type == JPEG_BLIP) {
        //FIXIMAGE
