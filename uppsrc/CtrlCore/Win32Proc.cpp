...
//      return MAKELONG(0, MNC_SELECT);
    ...
    case WM_SYSKEYUP: {
        ...
        else if(message == WM_SYSKEYDOWN /*&& ((lParam & 0x20000000) || wParam == VK_F10)*/)
            keycode = KEYtoK((dword)wParam);
        else if(message == WM_SYSKEYUP /*&& ((lParam & 0x20000000) || wParam == VK_F10)*/)
            keycode = KEYtoK((dword)wParam) | K_KEYUP;
        else if(message == WM_CHAR && wParam != 127 && wParam > 32 || wParam == 32 && KEYtoK(VK_SPACE) == K_SPACE) {
            if(IsWindowUnicode(hwnd))     // TRC 04/10/17: ActiveX Unicode patch
                keycode = (dword)wParam;
            else {
                char b[20];
                ::GetLocaleInfoA(MAKELCID(LOWORD(GetKeyboardLayout(0)), SORT_DEFAULT),
    ...
    case WM_SETFOCUS:
        ...
        if(this != focusCtrlWnd) {
            if(IsEnabled()) {
                ...
                }else if(lastActiveWnd && lastActiveWnd->IsEnabled()) {
