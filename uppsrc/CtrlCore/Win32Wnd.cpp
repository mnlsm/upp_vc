...
static BOOL CALLBACK sDumpWindow(HWND hwnd, LPARAM lParam) {
    ...
    if(ctrl) {
        ...
        ::GetClassNameA(hwnd, clsname, __countof(clsname));
        ::GetWindowTextA(hwnd, title, __countof(title));
...
DWORD WINAPI Ctrl::Win32OverwatchThread(LPVOID) {
    ...
    unsigned short ag = RegisterClassA(&wc);

    OverwatchHWND = CreateWindowExA(0, "UPP-OVERWATCH", NULL, WS_OVERLAPPEDWINDOW | WS_DISABLED,
...
static void Win32PanicMessageBox(const char *title, const char *text) {
    MessageBoxA(::GetActiveWindow(), text, title, MB_ICONSTOP | MB_OK | MB_APPLMODAL);
}
...
#define ILOG(x) // RLOG(x)
    ...
    InitTimer();
    ILOG("SetTimer");
    utilityHWND = CreateWindowA(L_("UPP-TIMER"), NULL, WS_OVERLAPPED | WS_DISABLED,
...
static PSLWA SetLayeredWindowAttributes() {
    ...
    if(!inited) {
        inited = true;
        if(HMODULE hDLL = LoadLibraryA("user32"))
...
void Ctrl::Create0(Ctrl::CreateBox *cr) {
    ...
    if(IsWinNT() && (!cr->parent || IsWindowUnicode(cr->parent))) {
        top_->hwnd = CreateWindowExW(cr->exstyle,
                                     cr->savebits ? cr->dropshadow ? L"UPP-CLASS-SB-DS-W" : L"UPP-CLASS-SB-W"
                             : cr->dropshadow ? L"UPP-CLASS-DS-W"    : L"UPP-CLASS-W",
                                     L"", cr->style, 0, 0, 0, 0,
                                     cr->parent, NULL, hInstance, this);
    }else {
        top_->hwnd = CreateWindowExA(cr->exstyle,
                                     cr->savebits ? cr->dropshadow ? "UPP-CLASS-SB-DS-A" : "UPP-CLASS-SB-A"
                             : cr->dropshadow ? "UPP-CLASS-DS-A"    : "UPP-CLASS-A",
                                     "", cr->style, 0, 0, 0, 0,
                                     cr->parent, NULL, hInstance, this);
    ...
    ::MoveWindow(top_->hwnd, r.left, r.top, r.Width(), r.Height(), false);   // To avoid "black corners" artifact effect
    ::ShowWindow(top_->hwnd, visible ? cr->show : SW_HIDE);
//  ::UpdateWindow(hwnd);
    StateH(OPEN);
    LLOG(EndIndent << "//Ctrl::Create in " << UPP::Name(this));
    RegisterDragDrop(top_->hwnd, (LPDROPTARGET)(top_->dndtgt = NewUDropTarget(this)));
    CancelMode();
    RefreshLayoutDeep();
}
...
void Ctrl::WndFree() {
    ...
    ReleaseUDropTarget(top_->dndtgt);
    isopen = false;
    if(!top_)
        return;
    HWND owner = GetWindow(top_->hwnd, GW_OWNER);   // CXL 31.10.2003 z DoRemove
    bool focus = ::GetFocus() == top_->hwnd;
...
void Ctrl::SetAlpha(byte alpha) {
    GuiLock __;
    HWND hwnd = GetHWND();
    if(!IsAlphaSupported() || parent_ || !top_ || !hwnd)
        return;
    if(alpha == 255) {
...
Rect Ctrl::GetDefaultWindowRect() {
    HWND hwnd = ::CreateWindowA("UPP-CLASS-A", NULL, WS_OVERLAPPED,
                                CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT,
                                NULL, NULL, NULL, NULL);
    Rect sr;
    if(hwnd) {
