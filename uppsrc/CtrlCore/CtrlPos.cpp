...
void Ctrl::Lay1(int& _pos, int& r, int align, int a, int b, int sz) const {
    _pos = a;
    int size = b;
    switch(align) {
    case CENTER:
...
Rect Ctrl::CalcRect(LogPos _pos, const Rect& prect, const Rect& pview) const {
    Rect r;
    Size sz = InFrame() ? prect.Size() : pview.Size();
    Lay1(r.left, r.right, _pos.x.GetAlign(),
         PosVal(_pos.x.GetA()).cx, PosVal(_pos.x.GetB()).cx, sz.cx);
    Lay1(r.top, r.bottom, _pos.y.GetAlign(),
         PosVal(_pos.y.GetA()).cy, PosVal(_pos.y.GetB()).cy, sz.cy);
    return r;
}
...
Rect Ctrl::GetScreenRect() const {
    ...
    Rect r = GetRect();
    if(parent_) {
        Rect pr = inframe_ ? parent_->GetScreenRect() : parent_->GetScreenView();
        r = r + pr.TopLeft();
    }else if(activex) {
        r = GetWndScreenRect();
    }
...
Rect  Ctrl::GetVisibleScreenRect() const {
    ...
    Rect r = GetRect();
    if(parent_) {
        Rect pr = inframe_ ? parent_->GetVisibleScreenRect() : parent_->GetVisibleScreenView();
        Rect pr1 = inframe_ ? parent_->GetScreenRect() : parent_->GetScreenView();
        r = (r + pr1.TopLeft()) & pr;
    }else if(activex)
        r = GetWndScreenRect();
...
void Ctrl::SetPos0(LogPos p, bool _inframe) {
    GuiLock __;
    if(p == pos && inframe_ == _inframe)
        return;
    if(parent_) {
        Rect from = GetRect().Size();
        Top *top = GetTopRect(from, true)->top_;
        if(top) {
...
void Ctrl::UpdateRect0() {
    ...
    LTIMING("UpdateRect0");
    if(parent_) {
        rect = CalcRect(parent_->GetRect(), parent_->GetView());
    }else {
        Rect r = GetWorkArea();
        rect = CalcRect(r, r);
    }
