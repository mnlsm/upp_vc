...
void Ctrl::DoKillFocus(Ptr<Ctrl> pfocusCtrl, Ptr<Ctrl> nfocusCtrl) {
    ...
    if(pfocusCtrl && pfocusCtrl->parent_ && !pfocusCtrl->parent_->destroying)
        pfocusCtrl->parent_->ChildLostFocus();
    SyncCaret();
}
...
void Ctrl::DoSetFocus(Ptr<Ctrl> pfocusCtrl, Ptr<Ctrl> nfocusCtrl, bool activate) {
    ...
    if(focusCtrl == nfocusCtrl && nfocusCtrl && nfocusCtrl->parent_ &&
            !nfocusCtrl->parent_->destroying)
        nfocusCtrl->parent_->ChildGotFocus();
    SyncCaret();
}
...
void Ctrl::SyncCaret() {
    ...
    if(focusCtrl && focusCtrl->IsVisible()) {
        bool focus_inframe = focusCtrl->InFrame();
        ...
        for(Ctrl *q = focusCtrl->GetParent(); q; q = q->GetParent()) {
            cr &= focus_inframe ? q->GetScreenRect() : q->GetScreenView();
            focus_inframe = q->InFrame();
        }
    }
...
bool  Ctrl::HasFocusDeep() const {
    GuiLock __;
    if(HasFocus() || HasChildDeep(FocusCtrl()))
        return true;
