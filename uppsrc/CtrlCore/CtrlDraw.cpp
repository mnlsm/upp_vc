...
void Ctrl::RefreshFrame(const Rect& r) {
    ...
    if(!top_) {
        if(InFrame())
            parent_->RefreshFrame(r + GetRect().TopLeft());
        else
            parent_->Refresh(r + GetRect().TopLeft());
    }else {
...
void Ctrl::Refresh() {
    GuiLock __;
    if(fullrefresh || !IsVisible() || !IsOpen())
        return;
...
void  Ctrl::ScrollRefresh(const Rect& r, int dx, int dy) {
    ...
    if(dx)
        WndInvalidateRect(RectC(dx >= 0 ? r.left : r.right - tdx, r.top - tdy, tdx, r.Height()));
    if(dy)
        WndInvalidateRect(RectC(r.left - tdx, dy >= 0 ? r.top : r.bottom - tdy, r.Width(), tdy));
}
...
bool Ctrl::AddScroll(const Rect& sr, int dx, int dy) {
    ...
    if(!top_)
        return true;
    for(int i = 0; i < top_->scroll.GetCount(); i++) {
        Scroll& sc = top_->scroll[i];
...
void  Ctrl::ScrollView(const Rect& _r, int dx, int dy) {
    ...
    Rect r = _r & vsz;
    Ctrl *w = NULL;
    for(w = this; w->parent_; w = w->parent_) {
        if(w->InFrame()) {
            Refresh();
            return;
        }
    ...
    if(w->AddScroll(sr, dx, dy)) {
        ...
        Top *top = GetTopCtrl()->top_;
        for(Ctrl *q = GetFirstChild(); q; q = q->GetNext()) {
            if(q->InView())
                ScrollCtrl(top, q, r, q->GetRect(), dx, dy);
        }
        if(parent_) {
            for(Ctrl *q = parent_->GetFirstChild(); q; q = q->GetNext()) {
                if(q->InView() && q != this)
...
void  Ctrl::SyncScroll() {
    ...
    if(!top_)
        return;
    Vector<Scroll> scroll = top_->scroll;
    top_->scroll.Clear();
    if(IsFullRefresh())
        return;
...
bool Ctrl::PaintOpaqueAreas(SystemDraw& w, const Rect& r, const Rect& clip, bool nochild) {
    ...
    for(Ctrl *q = lastchild; q; q = q->prev) {
        if(!q->PaintOpaqueAreas(w, q->GetRect() + (q->InView() ? viewpos : off), q->InView() ? cview : clip))
            return false;
    }
    ...
    for(Ctrl *q = GetFirstChild(); q; q = q->GetNext()) {
        if(q->IsVisible() && GetScreenView().Contains(q->GetScreenRect())) {
            ...
            if((q->GetOpaqueRect() + sw.TopLeft()).Contains(clip)) {
                ...
                if(h) w = h;
            }else if(q->GetScreenView().Contains(clip))
                w = q->FindBestOpaque(clip);
            else if(q->GetScreenRect().Intersects(clip))
                w = NULL;
        }
    }
    return w;
...
Ctrl *Ctrl::GetTopRect(Rect& r, bool _inframe) {
    GuiLock __;
    if(!_inframe) {
        r &= Rect(GetSize());
    ...
    if(parent_) {
        ...
        return parent_->GetTopRect(r, InFrame());
    }
    return this;
}

void  Ctrl::DoSync(Ctrl *q, Rect r, bool _inframe) {
    GuiLock __;
    ASSERT(q);
    LLOG("DoSync " << UPP::Name(q) << " " << r);
    Ctrl *top = q->GetTopRect(r, _inframe);
    top->SyncScroll();
    top->WndUpdate(r);
}
...
void  Ctrl::Sync() {
    ...
    if(top_ && IsOpen()) {
        ...
        DoSync(parent_, GetRect(), inframe_);
    }
    SyncCaret();
}
...
void Ctrl::DrawCtrlWithParent(Draw& w, int x, int y) {
    ...
    if(parent_) {
        Rect r = GetRect();
        Ctrl *top = parent_->GetTopRect(r, inframe_);
...
void Ctrl::SyncMoves() {
    ...
    if(!top_)
        return;
    for(int i = 0; i < top_->move.GetCount(); i++) {
        MoveCtrl& m = top_->move[i];
        if(m.ctrl) {
            RefreshFrame(m.from);
            RefreshFrame(m.to);
        }
    }
    for(int i = 0; i < top_->scroll_move.GetCount(); i++) {
        MoveCtrl& s = top_->scroll_move[i];
        if(s.ctrl) {
            RefreshFrame(s.from);
            RefreshFrame(s.to);
        }
