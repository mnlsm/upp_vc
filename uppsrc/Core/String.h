...
inline int strlen__(const char *s) {
    return s ? (int)strlen(s) : 0;
}
inline int strlen__(const wchar *s) {
    return s ? (int)wstrlen(s) : 0;
}

inline int cmpval__(char x) {
    return (byte)x;
}
inline int cmpval__(wchar x) {
    return (word)x;
}
...
public:
    ...
    const tchar *End() const {
        return B::Begin() + GetLength();
    }
    const tchar *Last() const {
        return End() - !!B::GetCount();
    }
    const tchar *GetIter(int i) const {
        ASSERT(i >= 0 && i <= B::GetCount());
        return B::Begin() + i;
    }
    ...
    void  Cat(const tchar *s, int len) {
        B::Cat(s, len);
    }
    ...
    void  Cat(const tchar *s, const tchar *lim) {
        ASSERT(s <= lim);
        Cat(s, int(lim - s));
    }
    void  Cat(const String& s, int len) {
        B::Cat(~s, len);
    }
    void  Cat(const bchar *s, int len) {
        Cat((const tchar *) s, len);
    }
    ...
    bool   IsEqual(const tchar *s) const {
        return Compare(s) == 0;
    }

    String Mid(int pos, int length) const;
    String Mid(int pos) const {
        return Mid(pos, GetLength() - pos);
    }
    ...
    int    ReverseFind(const String& s, int from) const {
        return ReverseFind(s.GetCount(), ~s, from);
    }
    int    ReverseFind(const tchar *s) const {
        return ReverseFind(s, GetLength() - 1);
    }
    ...
    void   Replace(const tchar *find, const tchar *replace) {
        Replace(find, strlen(find), replace, strlen(replace));
    }
    ...
    bool   StartsWith(const String& s) const {
        return StartsWith(~s, s.GetLength());
    }
    ...
    bool   EndsWith(const String& s) const {
        return EndsWith(~s, s.GetLength());
    }

    int    FindFirstOf(int len, const tchar *s, int from) const;
    int    FindFirstOf(const tchar *s, int from = 0) const;
    int    FindFirstOf(const String& s, int from = 0) const {
        return FindFirstOf(s.GetCount(), ~s, from);
    }

    friend bool operator<(const String& a, const String& b) {
        return a.Compare(b) < 0;
    }
    friend bool operator<(const String& a, const tchar *b) {
        return a.Compare(b) < 0;
    }
    friend bool operator<(const tchar *a, const String& b) {
    ...
    friend String operator+(const String& a, const tchar *b) {
        String c(a);
        c += b;
        return c;
    }
    friend String operator+(const tchar *a, const String& b) {
        String c(a);
        c += b;
        return c;
    }
    ...
    friend String operator+(const String& a, tchar b) {
        String c(a);
        c += b;
        return c;
    }
    friend String operator+(tchar a, const String& b) {
        String c(a, 1);
        c += b;
        return c;
    }
...
class String0 : Moveable<String0> {
    ...
    bool   IsSmall() const {
        return chr[KIND] == SMALL;
    }
    bool   IsLarge() const {
        return chr[KIND] != SMALL;
    }
    bool   IsMedium() const {
        return chr[KIND] == MEDIUM;
    }
    bool   IsRef() const {
        return (byte)chr[KIND] > MEDIUM;
    }
    Rc    *Ref() const {
        return (Rc *)ptr - 1;
    }
    bool   IsShared() const {
        return Ref()->refcount != 1;
    }
    bool   IsSharedRef() const {
        return IsRef() && IsShared();
    }
    int    LAlloc() const {
        int b = (byte)chr[KIND];
        return b == 255 ? Ref()->alloc : b;
    }
...
public:
    ...
    int GetCount() const {
        return IsSmall() ? chr[SLEN] : w[LLEN];
    }
    int GetLength() const {
        return GetCount();
    }
    ...
    String0& operator=(const String0& s) {
        Free();
        Set(s);
        return *this;
    }
...
public:
    ...
    String(const byte *s, int n) {
        String0::Set((const char *)s, n);
    }
    String(const char *s, const char *lim) {
        String0::Set(s, (int)(lim - s));
    }
    ...
    String(const std::string& s) {
        String0::Set(s.c_str(), (int)s.length());
    }
...
template<>
inline String AsString(const String& s) {
    return s;
}

template<>
inline unsigned GetHashValue(const String& s) {
    return memhash(~s, s.GetLength());
}
...
protected:
    ...
    void    Free();
    void    Swap(WString0& b) {
        Upp::Swap(ptr, b.ptr);
        Upp::Swap(length, b.length);
        Upp::Swap(alloc, b.alloc);
        Dsyn();
        b.Dsyn();
    }
...
public:
    ...
    unsigned GetHashValue() const {
        return memhash(ptr, length * sizeof(wchar));
    }
    bool     IsEqual(const WString0& s) const {
        return s.length == length && memcmp(ptr, s.ptr, length * sizeof(wchar)) == 0;
    }
...
public:
    ...
    const WString& operator+=(wchar c) {
        Cat(c);
        return *this;
    }
    const WString& operator+=(const wchar *s) {
        Cat(s);
        return *this;
    }
    const WString& operator+=(const WString& s) {
        Cat(s);
        return *this;
    }
    ...
    WString& operator=(WStringBuffer& b) {
        *this = WString(b);
        return *this;
    }
    WString& operator<<=(const WString& s) {
        if(this != &s) {
            WString0::Free();
            WString0::Set(s, s.GetCount());
        }
        return *this;
    }

    void   Shrink() {
        *this = WString(Begin(), GetLength());
    }
    ...
    friend WString operator+(char a, const WString& b) {
        WString c(a, 1);
        c += b;
        return c;
    }
...
#ifndef _HAVE_NO_STDWSTRING
inline std::wstring to_string(const WString& s) {
...
inline bool  IsEmpty(const WString& s) {
    return s.IsEmpty();
}

template<>
inline bool  IsNull(const WString& s) {
    return s.IsEmpty();
}
...
template<>
inline unsigned GetHashValue(const WString& s) {
    return memhash(~s, 2 * s.GetLength());
}
...
template<> inline String AsString(const char * const &s) {
    return s;
}
template<> inline String AsString(char * const &s) {
    return s;
}
template<> inline String AsString(const char& a) {
    return String(a, 1);
}
template<> inline String AsString(const signed char& a) {
    return String(a, 1);
}
template<> inline String AsString(const unsigned char& a) {
    return String(a, 1);
}
template<> inline String AsString(const bool& a) {
    return a ? "true" : "false";
}
