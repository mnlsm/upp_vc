...
protected:
    ...
    int      Ix(int i) const {
        return i + start < alloc ? i + start : i + start - alloc;
    }
    ...
    T       *AddHead0() {
        AssertMoveable<T>();
        Add0();
        return &vector[start = Ix(alloc - 1)/*(start + alloc - 1) % alloc*/];
    }
    T       *AddTail0() {
        AssertMoveable<T>();
        Add0();
...
public:
    ...
    T&       AddHead() {
        return *new(AddHead0()) T;
    }
    T&       AddTail() {
        return *new(AddTail0()) T;
    }
    ...
    void     DropHead(int n) {
        while(n-- > 0) BiVector<T>::DropHead();
    }
    void     DropTail(int n) {
        while(n-- > 0) BiVector<T>::DropTail();
    }
...
public:
    ...
    void     AddHead(const T& x) {
        bv.AddHead(DeepCopyNew(x));
    }
    void     AddTail(const T& x) {
        bv.AddTail(DeepCopyNew(x));
    }
    void     AddHeadPick(pick_ T& x) {
        bv.AddHead(new T(x));
    }
    ...
    template <class TT> TT& CreateHead() {
        TT *q = new TT;
        bv.AddHead(q);
        return *q;
    }
    template <class TT> TT& CreateTail() {
        TT *q = new TT;
        bv.AddTail(q);
        return *q;
    }
    ...
    const T& Head() const {
        return *(const T *) bv.Head();
    }
    const T& Tail() const {
        return *(const T *) bv.Tail();
    }
    void     DropHead() {
        delete(T*) bv.Head();
        bv.DropHead();
    ...
    void operator=(pick_ BiArray& src) {
        Free();
        bv = src.bv;
    }
    BiArray()                                {}
