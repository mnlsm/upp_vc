...
public:
    ...
    WString         GetIndexLetter(const wchar *text) const {
        return (*getindexletter)(text, language);
    }
    int             Compare(const wchar *a, int alen, const wchar *b, int blen) const {
        return (*compare)(a, alen, b, blen, language);
    }

    int             Compare(const wchar *a, const wchar *b) const {
        return Compare(a, wstrlen(a), b, wstrlen(b));
    }
    int             Compare(WString a, WString b) const {
        return Compare(a, a.GetLength(), b, b.GetLength());
    }
    int             Compare(const char *a, const char *b) const {
        return Compare(ToUnicode(a, CHARSET_DEFAULT), ToUnicode(b, CHARSET_DEFAULT));
    }
    int             Compare(String a, String b) const {
        return Compare(a.ToWString(), b.ToWString());
    }
...
//BWC
    ...
    int             Compare(const wchar *a, const wchar *b, int alen, int blen) const {
        return Compare(a, alen, b, blen);
    }
};
