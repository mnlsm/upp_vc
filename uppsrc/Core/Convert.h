...
inline unsigned stou(const byte *ptr, void *endptr = NULL, unsigned base = 10) {
    return stou((const char *)ptr, endptr, base);
}
...
inline int    StrInt(const char* s) {
    return ScanInt(s);
}
inline double IntDbl(int i) {
    return IsNull(i) ? double(Null) : double(i);
}
inline int    DblInt(double d) {
    return IsNull(d) ? int(Null) : fround(d);
}
Value         StrDblValue(const char* s);
inline double StrDbl(const char* s) {
    return ScanDouble(s);
}
...
struct ConvertInt64 : public ConvertInt {
    ConvertInt& MinMax(int64 _min, int64 _max) {
        minval = _min;
        maxval = _max;
        return *this;
    }
    ConvertInt& Min(int64 _min) {
        minval = _min;
        return *this;
    }
    ConvertInt& Max(int64 _max) {
        maxval = _max;
        return *this;
    }
    int64       GetMin() const {
...
public:
    ...
    MapConvert&  Add(const Value& a, const Value& b) {
        map.Add(a, b);
        return *this;
    }
