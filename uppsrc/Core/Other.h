...
public:
    ...
    One(const One<T>& p, int) {
        ptr = p.IsEmpty() ? NULL : DeepCopyNew(*p);
    }
...
class Any : Moveable<Any> {
    ...
    void Pick(pick_ Any& s) {
        ptr = s.ptr;
        const_cast<Any&>(s).ptr = (BaseData *)1;
    }
...
public:
    ...
    template <class T> bool Is() const {
        return ptr && ptr->typeno == StaticTypeNo<T>();
    }
...
protected:
    ...
    void LPN(int i) {
        link_prev[i]->link_next[i] = link_next[i]->link_prev[i] = (T *)this;
    }
...
public:
    ...
    const T* GetNext(int i = 0) const {
        return link_next[i];
    }
    const T* GetPrev(int i = 0) const {
        return link_prev[i];
    }

    void LinkSelf(int i = 0) {
        link_next[i] = link_prev[i] = (T *)this;
    }
    void LinkSelfAll() {
        for(int i = 0; i < N; i++)
            LinkSelf(i);
    ...
    void Unlink(int i = 0) {
        link_next[i]->link_prev[i] = link_prev[i];
        link_prev[i]->link_next[i] = link_next[i];
        LinkSelf(i);
    }
    void UnlinkAll() {
        for(int i = 0; i < N; i++)
    ...
    void LinkBefore(Link *n, int i = 0) {
        link_next[i] = (T *)n;
        link_prev[i] = link_next[i]->link_prev[i];
        LPN(i);
    }
    void LinkAfter(Link *p, int i = 0) {
        link_prev[i] = (T *)p;
        link_next[i] = link_prev[i]->link_next[i];
        LPN(i);
    }

    T* InsertNext(int i = 0) {
        T *x = new T;
        x->LinkAfter(this, i);
        return x;
    }
    T* InsertPrev(int i = 0) {
        T *x = new T;
        x->LinkBefore(this, i);
        return x;
    }

    void DeleteList(int i = 0) {
        while(link_next[i] != GetPtr())
            delete link_next[i];
    }

    bool InList(int i = 0) const {
        return link_next[i] != GetPtr();
    }
...
private:
    ...
    struct Key : Moveable<Key> {
        ...
        unsigned GetHashValue() const {
            return CombineHash(key, (uintptr_t)type);
        }
    };
