...
public:
    ...
    const byte *Peek(int size = 1) {
        ASSERT(size > 0);
        return ptr + size <= rdlim ? ptr : NULL;
    }
    ...
    int       Get8() {
        return ptr < rdlim ? *ptr++ : _Get8();
    }
#ifdef CPU_X86
    int       Get16() {
        if(ptr + 1 >= rdlim) return _Get16();
        int q = *(word *)ptr;
        ptr += 2;
        return q;
    }
    int       Get32() {
        if(ptr + 3 >= rdlim) return _Get32();
        int q = *(dword *)ptr;
        ptr += 4;
        return q;
    }
    int64     Get64() {
        if(ptr + 7 >= rdlim) return _Get64();
        int64 q = *(int64 *)ptr;
        ptr += 8;
        return q;
    }
    ...
    dword     GetW(wchar *s, int count) {
        return Get(s, count * 2) / 2;
    }
    bool      GetAllW(wchar *s, int count) {
        return GetAll(s, count * 2);
    }
...
//  Stream as serialization streamer
    void      SetLoading() {
        ASSERT(style & STRM_READ);
        style |= STRM_LOADING;
    }
    void      SetStoring() {
        ASSERT(style & STRM_WRITE);
        style &= ~STRM_LOADING;
    }
    bool      IsLoading() {
        return style & STRM_LOADING;
    }
    bool      IsStoring() {
        return !IsLoading();
    }
...
public:
    ...
    bool Open(const char *fn) {
        return FileStream::Open(fn, FileStream::CREATE);
    }
...
public:
    bool Open(const char *fn) {
        return FileStream::Open(fn, FileStream::APPEND);
    }
...
public:
    ...
    int64       GetRawOffset() const {
        return rawoffset;
    }
    dword       GetRawCount() const {
        return rawsize;
    }
    ...
    const byte *GetIter(int i) const {
        ASSERT(IsOpen() && i >= 0 && (dword)i <= size);
        return base + i;
    }
...
inline int64 CopyStream(Stream& dest, Stream& src) {
    return CopyStream(dest, src, src.GetLeft());
}
