...
public:
    ...
    String  PreservedText(const String& s, byte charset = CHARSET_DEFAULT) {
        return PreservedText(~s, charset);
    }
...
public:
    ...
    int    GetAttrCount() const {
        return attr.GetCount() + !IsNull(attr1);
    }
    String GetAttr(int i) const {
        return i ? attr.GetKey(i - 1) : attr1;
    }
    String operator[](int i) const {
        return i ? attr[i - 1] : attrval1;
    }
    String operator[](const char *id) const {
        return attr1 == id ? attrval1 : attr.Get(id, Null);
    }
    ...
    bool   IsDecl();
    String PeekDecl() {
        return ReadDecl(false);
    }
    String ReadDecl() {
        return ReadDecl(true);
    }
    ...
    void   PreserveAllWhiteSpaces(bool b = true) {
        preserveall = b;
    }
...
public:
    ...
    bool           IsEmpty() const {
        return type == XML_DOC && node.GetCount() == 0;
    }
    operator bool() const {
        return !IsEmpty();
    }
    ...
    const XmlNode& operator[](int i) const {
        return i >= 0 && i < node.GetCount() ? node[i] : Void();
    }
