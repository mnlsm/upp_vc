...
//  const T& operator()(const K& k, const T& d) const { return Get(k, d); }
    ...
    int      UnlinkKey(const K& k, unsigned h) {
        return key.UnlinkKey(k, h);
    }
    int      UnlinkKey(const K& k) {
        return key.UnlinkKey(k);
    }
    bool     IsUnlinked(int i) const {
        return key.IsUnlinked(i);
    }
    void     Sweep();

    T&       Insert(int i, const K& k) {
        key.Insert(i, k);
        return value.Insert(i);
    }
    ...
    int      GetAlloc() const {
        return value.GetAlloc();
    }

    unsigned GetHash(int i) const {
        return key.GetHash(i);
    }
...
//  T        Pop()                                 { T h = Top(); Drop(); return h; }
    ...
    bool             IsPicked() const {
        return value.IsPicked() || key.IsPicked();
    }
    ...
    friend int     GetCount(const AMap& v) {
        return v.GetCount();
    }
};
...
public:
    ...
    friend void    Swap(VectorMap& a, VectorMap& b) {
        a.B::Swap(b);
    }
...
public:
    ...
    template <class TT> TT& Create(const K& k) {
        TT *q = new TT;
        B::key.Add(k);
        B::value.Add(q);
        return *q;
    ...
    T        *PopDetach() {
        B::key.Drop();
        return B::value.PopDetach();
    }
    T        *Detach(int i) {
        B::key.Remove(i);
        return B::value.Detach(i);
    }
    ...
    friend void    Swap(ArrayMap& a, ArrayMap& b) {
        a.B::Swap(b);
    }
...
public:
    ...
    friend void Swap(SegtorMap& a, SegtorMap& b) {
        a.B::Swap(b);
    }
