...
struct PaletteCv {
    ...
    byte  Get(const RGBA& b) const {
        return cv[(int(b.r >> RASTER_SHIFT_R) << 10) +
                  (int(b.g >> RASTER_SHIFT_G)) +
                  (int(b.b >> RASTER_SHIFT_B) << 6)];
    }
    PaletteCv() {
        cv.Alloc(RASTER_MAP_R * RASTER_MAP_G * RASTER_MAP_B);
    }
};
...
public:
    ...
    void Set1mf() {
        type = RASTER_1 | RASTER_MSBFIRST;
    }
    void Set2lf() {
        type = RASTER_2;
    }
    void Set2mf() {
        type = RASTER_2 | RASTER_MSBFIRST;
    }
    void Set4lf() {
        type = RASTER_4;
    }
    void Set4mf() {
        type = RASTER_4 | RASTER_MSBFIRST;
    }
    void Set8() {
        type = RASTER_8;
    }
...
public:
    class Line {
        ...
        void               MakeRGBA() const;
        void Free() {
            if(free_) delete[] data;
            if(fmtfree) delete[] fmtdata;
        }
    ...
    public:
        ...
        Line(const RGBA *data, bool bfree)
            : data(data),
              fmtdata((byte *)data),
              raster(NULL),
              free_(bfree),
        ...
        Line(const byte *fmtdata, Raster *raster, bool fmtfree)
            : data(NULL), fmtdata(fmtdata), raster(raster), free_(false), fmtfree(fmtfree) {}
        Line(pick_ Line& b) {
            Pick(b);
        }
        Line() {
...
public:
    ...
    virtual Line               GetLine(int line);
    virtual int                GetPaletteCount() {
        return palette.GetCount();
    }
    virtual const RGBA         *GetPalette() {
        return palette.Begin();
    }
    virtual const RasterFormat *GetFormat() {
        return &format;
    }
...
class StreamRaster : public Raster {
    ...
    template <class T> static StreamRaster *FactoryFn() {
        return new T;
    }
...
public:
    ...
    static void Register() {
        AddFormat(&StreamRaster::FactoryFn<T>);
    }
