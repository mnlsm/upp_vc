...
#include <draw/tif/lib/tiffio.h>
#include <draw/tif/lib/tiffiop.h>
#undef int32
#undef uint32
...
extern "C" void _TIFFmemset(void* p, int v, tsize_t c) {
    memset(p, v, c);
}
extern "C" void _TIFFmemcpy(void* d, const tdata_t s, tsize_t c) {
    memcpy(d, s, c);
}
extern "C" int  _TIFFmemcmp(const tdata_t p1, const tdata_t p2, tsize_t c) {
...
#define BLT_PACK_11_4(o) dest[(o) + 0] = src[(o) + 0]; dest[(o) + 1] = src[(o) + 1]; \
    dest[(o) + 2] = src[(o) + 2]; dest[(o) + 3] = src[(o) + 3];
        ...
        case 0:
            break;
        case 1:
            *dest = (*src & 0x80) | (*dest | 0x7f);
            break;
        case 2:
            *dest = (*src & 0xc0) | (*dest | 0x3f);
            break;
        case 3:
            *dest = (*src & 0xe0) | (*dest | 0x1f);
            break;
        case 4:
            *dest = (*src & 0xf0) | (*dest | 0x0f);
            break;
        case 5:
            *dest = (*src & 0xf8) | (*dest | 0x07);
            break;
        case 6:
            *dest = (*src & 0xfc) | (*dest | 0x03);
            break;
        case 7:
            *dest = (*src & 0xfe) | (*dest | 0x01);
            break;
        }
    }else {
...
//      byte rshift = 8 - img.bitspersample;
        ...
        case 1:
            bpp = 1;
            put.contig = putContig1;
            format.Set1mf();
            break;
        case 2:
            bpp = 2;
            put.contig = putContig2;
            format.Set2mf();
            break;
        case 4:
            bpp = 4;
            put.contig = putContig4;
            format.Set4mf();
            break;
        case 8:
            format.Set8();
...
//  TIFFSetField(tiff, TIFFTAG_TRANSFERFUNCTION, gray);
    ...
    default:
        NEVER();
    case 24:
        format.Set24le(0xFF, 0xFF << 8, 0xFF << 16);
        break;
    case 32:
        format.Set32le(0xFF, 0xFF << 8, 0xFF << 16, 0xFF << 24);
        break;
    }
    if(bpp <= 8) {
