...
inline int  Grayscale(int r, int g, int b) {
    return (77 * r + 151 * g + 28 * b) >> 8;
}
inline int  Grayscale(const RGBA& c) {
    return Grayscale(c.r, c.g, c.b);
}
inline byte Saturate255(int x) {
    return byte(~(x >> 24) & (x | (-(x >> 8) >> 24)) & 0xff);
}
...
class ImageBuffer : NoCopy {
    ...
    RGBA*        Line(int i) const {
        ...
        return (RGBA *)~pixels + i * size.cx;
    }
    friend void  DropPixels___(ImageBuffer& b) {
        b.pixels.Clear();
    }
...
public:
    ...
    int   GetScanKind() const {
        return kind == IMAGE_UNKNOWN ? ScanKind() : kind;
    }
...
public:
    ...
    bool  IsSame(const Image& img) const {
        return GetSerialId() == img.GetSerialId();
    }
    ...
    bool IsNullInstance() const {
        Size sz = GetSize();
        return (sz.cx | sz.cy) == 0;
    }

    bool IsEmpty() const {
        return IsNullInstance();
    }
    operator Value() const {
        return RichValue<Image>(*this);
    }
