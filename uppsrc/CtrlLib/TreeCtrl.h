...
private:
    ...
    struct LineLess {
        bool   operator()(int y, const Line& l) const {
            return y < l.y;
        }
    };
...
public:
    ...
    int    GetChildCount(int id) const {
        return item[id].child.GetCount();
    }
    int    GetChild(int id, int i) const {
        return item[id].child[i];
    }
...
public:
    ...
    String GetLabel(int id) const {
        return option[id]->GetLabel();
    }

    int  Get(int id) const {
        return option[id]->Get();
    }
...
public:
    ...
    PopUpTree&   MaxHeight(int maxheight_) {
    ...
    PopUpTree&   AutoResize(bool b = true) {
        autosize = b;
        return *this;
    }
    PopUpTree&   SetDropLines(int n) {
        MaxHeight(n * (GetStdFontCy() + 4));
        return *this;
    }
};
...
public:
    ...
    DropTree& DisplayAll(bool b = true) {
        displayall = b;
        return *this;
    }
