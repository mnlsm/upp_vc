...
public:
    struct IdInfo {
        ...
        IdInfo& Accel() {
            return Accel(CharFilterDefaultToUpperAscii);
        }

        Value   GetInsertValue() {
            return insertgen ? insertgen->Get() : insertval;
        }
    ...
    public:
        ...
        Column& AddIndex(Id id) {
            arrayctrl->AddIndex(id);
            return Add(id);
        }
        Column& AddIndex() {
            Add(arrayctrl->GetIndexCount());
            arrayctrl->AddIndex();
            return *this;
        }
        ...
        Column& Ctrls(void (*factory)(One<Ctrl>&)) {
            return Ctrls(callback(factory));
        }
        template <class T>
        Column& Ctrls() {
            return Ctrls(DefaultCtrlFactory<T>());
        }
        Column& Ctrls(Callback2<int, One<Ctrl>&> factory);
        Column& Ctrls(void (*factory)(int, One<Ctrl>&)) {
            return Ctrls(callback(factory));
        }
...
private:
    ...
    struct CellInfo : Moveable<CellInfo> {
        ...
        CellCtrl&      GetCtrl() const {
            ASSERT(IsCtrl());
            return *(CellCtrl *)ptr.GetPtr();
        }
        ...
        bool           IsDisplay() const {
            return !ptr.GetBit() && ptr.GetPtr();
        }
        const Display& GetDisplay() const {
            ASSERT(IsDisplay());
            return *(const Display *)ptr.GetPtr();
        }
...
public: // temporary (TRC 06/07/28) // will be removed!
    Ctrl&  SetCtrl(int i, int j, Ctrl *newctrl) {
        return SetCtrl(i, j, newctrl, true, true);
    }
...
public:
    ...
    Column&                   ColumnAt(Id id) {
        return column[FindColumnWithId(id)];
    }
    HeaderCtrl::Column&       HeaderTab(int i) {
        return header.Tab(i);
    }
    HeaderCtrl::Column&       HeaderTab(Id id) {
        return header.Tab(FindColumnWithId(id));
    }
    const Column&             ColumnAt(int i) const {
        return column[i];
    }
    const Column&             ColumnAt(Id id) const {
        return column[FindColumnWithId(id)];
    }
    const HeaderCtrl::Column& HeaderTab(int i) const {
        return header.Tab(i);
    }
    const HeaderCtrl::Column& HeaderTab(Id id) const {
        return header.Tab(FindColumnWithId(id));
    }

    const HeaderCtrl&         HeaderObject() const {
        return header;
    }
    HeaderCtrl&               HeaderObject() {
        return header;
    }

    void       SerializeHeader(Stream& s) {
        header.Serialize(s);      // deprecated
    }
    ...
    bool       IsSelected(int i) const {
        return i < array.GetCount() && array[i].select;
    }
    void       ClearSelection();
    ...
    bool       IsLineDisabled(int i) const {
        return IsLineEnabled(i);
    }

    Vector<Value> ReadRow(int i) const;   // deprecated name
    Vector<Value> GetLine(int i) const {
        return ReadRow(i);
    }
...
//$+
    ...
    ArrayCtrl& NoAskRemove() {
        return AskRemove(false);
    }
    bool       IsAskRemove() const {
        return askremove;
    }

    ArrayCtrl& Header(bool b = true) {
        header.Invisible(!b);
        return *this;
    }
    ArrayCtrl& NoHeader() {
    ...
    ArrayCtrl& NoHorzGrid() {
        return HorzGrid(false);
    }
    ArrayCtrl& Grid(bool b = true) {
        return VertGrid(b).HorzGrid(b);
    }
    ArrayCtrl& NoGrid() {
        return Grid(false);
    }
    ...
    ArrayCtrl& PopUpEx(bool b = true) {
        popupex = b;
        return *this;
    }
    ArrayCtrl& NoPopUpEx() {
        return PopUpEx(false);
    }
    ArrayCtrl& NoFocusSetCursor() {
        focussetcursor = false;
        return *this;
    }
    ArrayCtrl& MovingHeader(bool b) {
        header.Moving(b);
        return *this;
    }
    ArrayCtrl& NoMovingHeader() {
        return MovingHeader(false);
    }
    ...
    ArrayCtrl& SetScrollBarStyle(const ScrollBar::Style& s) {
        sb.SetStyle(s);
        header.SetScrollBarStyle(s);
        return *this;
    }
    ArrayCtrl& SetHeaderCtrlStyle(const HeaderCtrl::Style& s) {
        header.SetStyle(s);
        return *this;
    }
...
public:
    ...
    void               Connect(ArrayCtrl& ac, Id id) {
        Connect(ac, ac.GetPos(id));
    }
    ...
    ArrayOption&       TrueFalse(Value _t, Value _f) {
        t = _t;
        f = _f;
        return *this;
    }
    ...
    ArrayOption&       ThreeState(bool b = true) {
        threestate = b;
        return *this;
    }
    ArrayOption&       NoThreeState() {
    ...
    ArrayOption&       HSwitch(bool hs = true) {
        hswitch = hs;
        return *this;
    }
    ArrayOption&       NoHSwitch() {
        return HSwitch(false);
    }
    bool               IsHSwitch() const {
        return hswitch;
    }

    ArrayOption&       VSwitch(bool vs = true) {
        vswitch = vs;
        return *this;
    }
    ArrayOption&       NoVSwitch() {
        return VSwitch(false);
    }
    bool               IsVSwitch() const {
        return vswitch;
    }

    ArrayOption&       Frame(bool frm = true) {
        frame = frm;
        return *this;
    }
    ArrayOption&       NoFrame() {
