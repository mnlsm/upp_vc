...
bool MenuBar::Key(dword key, int count) {
    ...
    }else if((horz ? key == K_LEFT : key == K_UP)) {
        Ctrl *ctrl = GetFocusChildDeep();
    ...
    }else if(parentmenu && !parentmenu->IsChild() && key == K_LEFT || key == K_ESCAPE) {
        if(HasMouseDeep())
            GetMouseCtrl()->Refresh();
        if(parentmenu && parentmenu->submenu)
            parentmenu->submenuitem->SetFocus();
        else if(IsChild() && HasFocusDeep()) {
            if(restorefocus)
...
DWORD WINAPI PlaySoundThread(LPVOID) {
    PlaySoundA("MenuPopup", NULL, SND_ASYNC | SND_NODEFAULT);
    return 0;
}
