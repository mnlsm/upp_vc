...
struct FileIconMaker : ImageMaker {
    ...
    virtual Image Make() const {
        ...
        for(int i = 0; i < 2; i++) {
            ...
            SHGetFileInfoA(ToSystemCharset(file), dir ? FILE_ATTRIBUTE_DIRECTORY : FILE_ATTRIBUTE_NORMAL,
                           &info, sizeof(info),
...
Image GetDriveImage(char drive_style) {
    switch(drive_style) {
    case FileSystemInfo::ROOT_NO_ROOT_DIR:
        return Null;
    case FileSystemInfo::ROOT_COMPUTER:
        return CtrlImg::Computer();
    case FileSystemInfo::ROOT_REMOVABLE:
        return CtrlImg::Diskette();
    case FileSystemInfo::ROOT_CDROM:
...
void FileSel::Open() {
    ...
    }else if(list.IsSelection())
        for(int i = 0; i < list.GetCount(); i++) {
            const FileList::File& m = list[i];
            if(!m.isdir) Finish();
        }
    else if(file.HasFocus()) {
