...
public:
    ...
    bool   CheckCharset(byte charset = CHARSET_DEFAULT) const {
        return GetInvalidCharPos(charset) < 0;
    }
    ...
    int    GetPos(int line) const {
        return GetPos(line, 0);
    }
    int    GetLine(int pos) const {
        return GetLinePos(pos);
    }
    ...
    void      Redo();
    bool      IsUndo() const {
        return undo.GetCount();
    }
    bool      IsRedo() const {
        return redo.GetCount();
    }
    void      ClearUndo() {
        undo.Clear();
        redo.Clear();
...
public:
    ...
    int    GetColumnLinePos(Point pos) const {
        return GetGPos(pos.y, pos.x);
    }
    ...
    LineEdit& SetScrollBarStyle(const ScrollBar::Style& s) {
        sb.SetStyle(s);
        return *this;
    }
