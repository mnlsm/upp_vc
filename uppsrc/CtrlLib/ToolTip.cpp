...
bool ToolTipHook(Ctrl *ctrl, bool _inframe, int event, Point p, int zdelta, dword keyflags) {
    if(event == Ctrl::MOUSEMOVE && ctrl != &AppToolTip()) {
        SyncToolTip(Ctrl::GetVisibleChild(ctrl, p, _inframe));
        LLOG("ToolTipHook MOUSEMOVE " << UPP::Name(ctrl) << " _inframe: " << _inframe);
    }
...
public:
    ...
    QTFPopUp& Width(int _width) {
        width = _width;
        return *this;
    }
    QTFPopUp& MaxHeight(int _maxheight) {
