...
void TabCtrl::Layout() {
    ...
    tabs.TopPos(0, th + style->sel.bottom).HSizePos(0, style->sel.left + style->sel.right);
    SyncTabs();
    ...
    if(sel < tab.GetCount())
        ScrollInto(sel);
    pane.VSizePos(style->tabheight + style->edge.top, style->edge.bottom).HSizePos(style->edge.left, style->edge.right);
