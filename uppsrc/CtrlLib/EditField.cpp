...
void EditField::Paint(Draw& w) {
    ...
    w.End();
    DrawTiles(w, dropcaret, CtrlImg::checkers());  //拖拽时显示的目标位置竖线
}
    ...
    if(AcceptText(d)) {
        ...
        if(GetSelection(sell, selh)) {
            if(c >= sell && c < selh) {
                ...
                c = sell;
            }else if(d.GetAction() == DND_MOVE && IsDragAndDropSource()) {
                if(c > sell)
                    c -= selh - sell;
                RemoveSelection();
                d.SetAction(DND_COPY);
...
void EditIntSpin::Inc() {
    ...
    if(IsNull(i)) {
        ...
        SetData(GetMin());
    }else if(!IsNull(GetMax()) && i < GetMax())
...
void EditIntSpin::Dec() {
    ...
    if(IsNull(i)) {
        ...
        SetData(GetMax());
    }else if(!IsNull(GetMin()) && i > GetMin())
...
EditDoubleSpin::EditDoubleSpin(double inc) : inc(inc) {
    Init();
}
EditDoubleSpin::EditDoubleSpin(double min, double max, double inc) : EditDouble(min, max), inc(inc) {
    Init();
}
