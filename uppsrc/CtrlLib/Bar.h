...
private:
    ...
    int&     HoVe(Size& sz) {
        return horz ? sz.cx : sz.cy;
    }
    int&     VeHo(Size& sz) {
        return horz ? sz.cy : sz.cx;
    }
    int&     HoVe(bool horz, Size& sz) const {
        return horz ? sz.cx : sz.cy;
    }
    int&     VeHo(bool horz, Size& sz) const {
        return horz ? sz.cy : sz.cx;
    }
...
public:
    ...
    void  AddGap(int gapsize) {
        Add(NULL, gapsize);
    }

    void  Margin(int v, int h) {
        vmargin = v;
        hmargin = h;
    }
...
public:
    ...
    Item&  AddMenu(const String& t, const UPP::Image& m, Callback c) {
        return AddMenu(t, m, c);
    }
...
public:
    ...
    BarCtrl& Top() {
        return Align(BAR_TOP);
    }
    BarCtrl& Bottom() {
        return Align(BAR_BOTTOM);
    }
    BarCtrl& Left() {
        return Align(BAR_LEFT);
    }
    BarCtrl& Right() {
        return Align(BAR_RIGHT);
    }
    int      GetAlign() const {
        return align;
    }
...
private:
    ...
    void     SetParentMenu(MenuBar *parent) {
        parentmenu = parent;
        style = parent->style;
    }
...
public:
    ...
    static void Execute(Callback1<Bar&> proc, Point p) {
        Execute(GetActiveCtrl(), proc, p);
    }
    static void Execute(Callback1<Bar&> proc) {
        Execute(proc, GetMousePos());
    }
...
public:
    ...
    ToolButton& MinSize(Size sz) {
        minsize = sz;
        return *this;
    }
    ToolButton& MaxIconSize(Size sz) {
        maxiconsize = sz;
        return *this;
    }
...
public:
    ...
    ToolBar& ButtonKind(int _kind) {
        kind = _kind;
        return *this;
    }
    ToolBar& AreaLook(int q = 1) {
        arealook = q;
        Refresh();
        return *this;
    }
...
public:
    StaticBarArea& UpperFrame(bool b) {
        upperframe = b;
        Refresh();
        return *this;
    }
    StaticBarArea& NoUpperFrame() {
        return UpperFrame(false);
    }
