...
public:
    ...
    public:
        ...
        Column&  Tip(const char *s) {
            tip = s;
            return *this;
        }
        Column&  SetPaper(Color c) {
            paper = c;
            return *this;
        }
...
public:
    ...
    HeaderCtrl&   Absolute();
    HeaderCtrl&   Fixed();
    HeaderCtrl&   SetStyle(const Style& s) {
        style = &s;
        Refresh();
        return *this;
    }
    HeaderCtrl&   Moving(bool b = true) {
        moving = b;
        return *this;
    }
    HeaderCtrl&   AutoHideSb(bool b = true) {
        autohidesb = b;
        SbTotal();
        return *this;
    }
    HeaderCtrl&   NoAutoHideSb() {
        return AutoHideSb(false);
    }
    HeaderCtrl&   HideSb(bool b = true) {
        sb.Show(!b);
        return *this;
    }

    HeaderCtrl&   SetScrollBarStyle(const ScrollBar::Style& s) {
        sb.SetStyle(s);
        return *this;
    }

    static int GetStdHeight() {
        return Draw::GetStdFontCy() + 4;
    }
