...
private:
    ...
    struct PaintRow : Moveable<PaintRow> {
        ...
        PaintCell& operator[](int i) {
            return cell[i];
        }
        const PaintCell& operator[](int i) const {
            return cell[i];
        }
    };
    ...
    struct Layout {
        ...
        PaintRow& operator[](int i) {
            return row[i];
        }
        const PaintRow& operator[](int i) const {
            return row[i];
        }
    };
    ...
    void             InvalidateRefresh(Point p) {
        InvalidateRefresh(p.y, p.x);
    }
...
public:
    ...
    Array<RichCell>&       operator[](int i) {
        return cell[i];
    }
    const Array<RichCell>& operator[](int i) const {
        return cell[i];
    }
    RichCell&       operator[](Point p) {
        return cell[p.y][p.x];
    }
    const RichCell& operator[](Point p) const {
        return cell[p.y][p.x];
    }
    int             GetCellPos(int i, int j) const;
    int             GetCellPos(Point p) const {
        return GetCellPos(p.y, p.x);
    }
    int             GetTableCount(int i, int j) const;
    int             GetTableCount(Point p) const {
        return GetTableCount(p.y, p.x);
    }
    Point           GetMasterCell(int i, int j) const;
    Point           GetMasterCell(Point p) const {
        return GetMasterCell(p.y, p.x);
    }
...
private:
    ...
    CellInfo      GetCellInfo(int i, int j) {
        return ci[i][j];
    }
