...
struct Zoom {
    ...
    int operator*(int x) const   {
        return d ? iscale(x, m, d) : 0;
    }
    int operator&(int x) const   {
        int q = d ? iscale(x, m, d) : 0;
        return x > 0 ? max(q, 1) : q;
    }
    ...
    friend int operator/(int x, Zoom z)  {
        return z.m ? iscale(x, z.d, z.m) : 0;
    }
};

inline bool IsNull(Zoom z) {
    return (z.m | z.d) == 0;
}
...
struct PageY : Moveable<PageY, RelOps<PageY> > {
    ...
    String ToString() const    {
        return AsString(page) + ":" + AsString(y);
    }
#endif
};
...
struct PageRect : public Rect {
    ...
    operator PageY() const               {
        return PageY(page, top);
    }
    PageRect& operator=(const Rect& r)   {
        (Rect&)(*this) = r;
        page = 0;
        return *this;
    }
    PageRect(const Rect& r)              {
        (Rect&)(*this) = r;
        page = 0;
    }
    PageRect()                           {
...
public:
    ...
    static int    FindType(const String& name)   {
        return Map().Find(name);
    }
    static RichObjectType& GetType(int i)        {
        return *Map()[i];
    }
    ...
    Size   GetDefaultSize(Size maxsize, void *context = NULL) const {
        return type ? type->GetDefaultSize(data, maxsize, context) : physical_size;
    }
    ...
    String GetLink(Point pt, Size sz, void *context = NULL) const {
        return type ? type->GetLink(data, pt, sz, context) : String();
    }
    ...
    String Write() const                         {
        return type ? type->Write(data) : (String)data;
    }
    bool   IsText() const                        {
        return type ? type->IsText() : false;
    }

    void   KeepRatio(bool b)                     {
        keepratio = b;
    }
    ...
    void   Menu(Bar& bar, void *context = NULL)  {
        if(type) type->Menu(bar, *this, context);
    }
    void   DefaultAction(void *context = NULL)   {
        if(type) type->DefaultAction(*this, context);
    }
...
struct RichCaret : PageRect {
    ...
    RichCaret()         {
        lineascent = caretascent = caretdescent = 0;
    }
};
...
inline int DotTwips(int dots) {
    return (dots * 12 +  5 * ROUNDOFF +  2) /  5 - ROUNDOFF;
}
inline int TwipDots(int twp)  {
    return (twp  *  5 + 12 * ROUNDOFF +  6) / 12 - ROUNDOFF;
}
inline int DotPoints(int dots) {
    return (dots *  3 + 25 * ROUNDOFF + 12) / 25 - ROUNDOFF;
}
inline int PointDots(int pts)  {
    return (pts  * 25 +  3 * ROUNDOFF +  1) /  3 - ROUNDOFF;
}
inline int TwipDotSize(int twp)  {
    return IsNull(twp) ? 0 : minmax<int>(TwipDots(twp), 0, MAX_DOTS);
}
inline int PointDotHeight(int p) {
    return (minmax<int>(Nvl(p, 0), 0, MAX_POINT_HEIGHT) * 25 + 5) / 6;
}
