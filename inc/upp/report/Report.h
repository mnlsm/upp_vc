...
public:
    ...
    Drawing               GetPage(int i) {
        Flush();
        return page[i];
    }
    Drawing               operator[](int i) {
        return GetPage(i);
    }
    const Array<Drawing>& GetPages() {
        Flush();
        return page;
    }
    ...
    Report&               operator<<(const char *qtf) {
        Put(qtf);
        return *this;
    }

    void                  SetRichTextLayoutTracer(RichTextLayoutTracer& l) {
        tracer = &l;
    }
    ...
    Report&               SetPageSize(int cx, int cy) {
        return SetPageSize(Size(cx, cy));
    }
    Report&               Landscape();
...
public:
    ...
    ReportView& Pages(int pags) {
        pages = pags;
        Layout();
        return *this;
    }
    ReportView& Numbers(bool nums) {
        numbers = nums;
        Refresh();
        return *this;
    }
