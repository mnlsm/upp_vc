...
template<>
inline String AsString(const Date& date) {
    return Format(date);
}
...
inline Time   operator+(Time a, double i) {
    return a +  int64(i);
}
inline Time   operator-(Time a, double i) {
    return a -  int64(i);
...
template<>
inline String AsString(const Time& time) {
    return Format(time);
}
