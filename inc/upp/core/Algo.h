...
struct StableSortIterator {
    ...
    Iter        operator + (int i) const {
        return Iter(ii + i, vi + i);
    }
    Iter        operator - (int i) const {
        return Iter(ii - i, vi - i);
    }
    int         operator - (Iter b) const {
        return (int)(ii - b.ii);
    }
    ...
    StableSortItem<T> operator*() const {
        return StableSortItem<T>(*ii, *vi);
    }

    friend void IterSwap(Iter a, Iter b) {
        IterSwap(a.ii, b.ii);
        IterSwap(a.vi, b.vi);
    }
...
struct IndexSort3Iterator {
    ...
    Iter        operator + (int i) const {
        return Iter(ii + i, vi + i, wi + i, xi + i);
    }
    Iter        operator - (int i) const {
        return Iter(ii - i, vi - i, wi - i, xi - i);
    }
...
struct StableSortOrderIterator : PostfixOps< StableSortOrderIterator<I, T> > {
    ...
    StableSortItem<T> operator*() const {
        return StableSortItem<T>(*(vi + *ii), *ii);
    }
...
public:
    ...
    bool operator()(const T& t1, const T& t2) const {
        return relation(t1.*member, t2.*member);
    }
};
