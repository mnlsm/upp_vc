...
class Vector : public MoveableAndDeepCopyOption< Vector<T> > {
    ...
    static void    RawFree(T *ptr) {
        if(ptr) MemoryFree(ptr);
    }
    static T      *RawAlloc(int& n);

#ifdef _DEBUG
    static Vector& SetPicked(pick_ Vector& v) {
        BreakWhenPicked((void *)&v);
        Vector& p = (Vector&)(v);
        p.items = -1;
        p.vector = NULL;
        return p;
...
public:
    T& Add() {
        Chk();
        if(items >= alloc) GrowF();
        return *(::new(vector + items++) T);
    }
    T& Add(const T& x) {
        Chk();
        return items < alloc ? DeepCopyConstruct(Rdd(), x) : GrowAdd(x);
    }
    T& AddPick(pick_ T& x) {
        Chk();
        return items < alloc ? *(::new(Rdd()) T(x)) : GrowAddPick(x);
    }
    void AddN(int n);
    ...
    T& At(int i) {
        if(i >= items) SetCountR(i + 1);
        return (*this)[i];
    }
    T& At(int i, const T& x) {
        if(i >= items) SetCountR(i + 1, x);
        return (*this)[i];
    }

    void Shrink() {
        if(items != alloc) ReAllocF(items);
    }
    void Reserve(int n);
    int GetAlloc() const {
        return alloc;
    ...
    void     Append(const Vector& x, int o, int c) {
        Insert(GetCount(), x, o, c);
    }
    void     AppendPick(pick_ Vector& x) {
        InsertPick(GetCount(), x);
    }
    int      GetIndex(const T& item) const;   //deprecated
    void     Swap(int i1, int i2) {
        UPP::Swap(Get(i1), Get(i2));
    }
...
// Optimizations
    ...
    friend void Append(Vector& dst, const Vector& src) {
        dst.Append(src);
    }
...
#ifdef _DEBUG
    void     Del(T **ptr, T **lim) {
        while(ptr < lim) delete(T *) *ptr++;
    }
    void     Init(T **ptr, T **lim) {
        while(ptr < lim) *ptr++ = new T;
    }
    void     Init(T **ptr, T **lim, const T& x) {
        while(ptr < lim) *ptr++ = DeepCopyNew(x);
    }
#else
    void     Del(void **ptr, void **lim) {
        while(ptr < lim) delete(T *) *ptr++;
    }
    void     Init(void **ptr, void **lim) {
        while(ptr < lim) *ptr++ = new T;
    }
    void     Init(void **ptr, void **lim, const T& x) {
        while(ptr < lim) *ptr++ = DeepCopyNew(x);
    }
#endif
...
public:
    ...
    T&       At(int i) {
        if(i >= GetCount()) SetCountR(i + 1);
        return Get(i);
    }
    T&       At(int i, const T& x) {
        if(i >= GetCount()) SetCountR(i + 1, x);
        return Get(i);
    }
    ...
    void     InsertPick(int i, pick_ Array& x) {
        vector.InsertPick(i, x.vector);
    }
    void     AppendPick(pick_ Array& x) {
        InsertPick(GetCount(), x);
    }
    int      GetIndex(const T& item) const;
    void     Swap(int i1, int i2) {
        UPP::Swap(vector[i1], vector[i2]);
    }
...
//  T        Pop()                      { T h = Top(); Drop(); return h; } // msvc bug
    ...
    Array& operator|(pick_ T& x) {
        AddPick(x);
        return *this;
    }

    bool     IsPicked() const {
        return vector.IsPicked();
    }
...
// Deep copy
    ...
    public:
        ...
        int  operator-(ConstIterator x) const {
            return (int)(ptr - x.ptr);
        }

        bool operator==(ConstIterator x) const {
            return ptr == x.ptr;
        }
        bool operator!=(ConstIterator x) const {
            return ptr != x.ptr;
...
// Standard container interface
    typedef T        ValueType;
    Iterator         Begin() {
        return (T **)vector.Begin();
    }
    Iterator         End() {
        return (T **)vector.End();
    }
    Iterator         GetIter(int pos) {
        return (T **)vector.GetIter(pos);
    }
    ConstIterator    Begin() const {
        return (T **)vector.Begin();
    }
...
private:
    static void IterSwap0(Iterator a, Iterator b) {
        UPP::Swap(*a.ptr, *b.ptr);
    }
public:
    friend void IterSwap(Iterator a, Iterator b) {
        Array<T>::IterSwap0(a, b);
    }
...
protected:
    ...
    void  Del(int n) {
        if(n < items) DoRange(n, items, DestroyArray);
    }
    void  Init(int n) {
        if(n > items) DoRange(items, n, ConstructArray);
        items = n;
    }
    void  Init(int n, const T& x) {
        if(n > items) Fill(items, n, x);
        items = n;
    }
    void  Free();
...
public:
    ...
    T&       Add(const T& x) {
        return DeepCopyConstruct(Add0(), x);
    }
    T&       AddPick(pick_ T& x) {
        return *(::new(Add0()) T(x));
    }
    ...
    void     Clear();
    T&       At(int i) {
        if(i >= items) SetCount(i + 1);
        return Get(i);
    }
    T&       At(int i, const T& x) {
        if(i >= items) SetCount(i + 1, x);
        return Get(i);
    }
    void     Shrink() {
        block.Shrink();
    ...
    void     Reserve(int xtra) {
        block.Reserve((xtra + NBLK - 1) / NBLK);
    }
    int      GetAlloc() const {
        return block.GetAlloc() * NBLK;
    }
...
// Standard container interface
    ...
    ConstIterator    End() const {
        return ConstIterator(*this, items);
    }
    ConstIterator    GetIter(int pos) const {
