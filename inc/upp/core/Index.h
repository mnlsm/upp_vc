...
struct PtrHash {
    unsigned operator()(const void *x) const {
        return GetHashValue((unsigned)(uintptr_t)x);
    }
};
...
public:
    ...
    bool     IsUnlinked(int i) const {
        return hash.IsUnlinked(i);
    }
    Vector<int> GetUnlinked() const {
        return hash.GetUnlinked();
    }
    void     Sweep();
...
// Optimalizations
    friend int  GetCount(const AIndex& v) {
        return v.GetCount();
    }
...
public:
    ...
    T       *PopDetach() {
        B::hash.Drop(1);
        return B::key.PopDetach();
    }
    T       *Detach(int i) {
        B::hash.Remove(i);
        return B::key.Detach(i);
    }
    ...
    friend void Swap(ArrayIndex& a, ArrayIndex& b) {
        a.B::Swap(b);
    }
