...
struct MtInspector {
    ...
    MtInspector(const char *s, int n = -1) {
        name = s;
        number = n;
        locked = blocked = 0;
    }
...
public:
    Semaphore& Get() {
        if(!ReadWithBarrier(semaphore)) Initialize();
        return *const_cast<Semaphore *>(semaphore);
    }
    operator Semaphore&() {
        return Get();
    }
...
inline int  AtomicInc(volatile Atomic& t) {
    return InterlockedIncrement((Atomic *)&t);
}
inline int  AtomicDec(volatile Atomic& t) {
    return InterlockedDecrement((Atomic *)&t);
}
inline int  AtomicXAdd(volatile Atomic& t, int incr) {
    return InterlockedExchangeAdd((Atomic *)&t, incr);
}
...
inline int  AtomicRead(const volatile Atomic& t) {
    return ReadWithBarrier(t);
}
inline void AtomicWrite(volatile Atomic& t, int val) {
    BarrierWrite(t, val);
}
...
public:
    Mutex& Get() {
        if(!ReadWithBarrier(section)) Initialize();
        return *const_cast<Mutex *>(section);
    }
    operator Mutex&() {
        return Get();
    }
...
public:
    RWMutex& Get() {
        if(!ReadWithBarrier(rw)) Initialize();
        return *const_cast<RWMutex *>(rw);
    }
    operator RWMutex&() {
        return Get();
    }
...
public:
    ConditionVariable& Get() {
        if(!ReadWithBarrier(cv)) Initialize();
        return *const_cast<ConditionVariable *>(cv);
    }
...
inline void AtomicWrite(volatile Atomic& t, int data) {
    t = data;
}
