...
public:
    ...
    LookFrame(const Value& v) : lookv(v) {
        look = NULL;
    }
    LookFrame() {
        look = NULL;
...
public:
    ...
    int     Insert(int pos, const String& text) {
        return Insert(pos, text.ToWString());
    }
    int     Insert(int pos, const char *text) {
        return Insert(pos, WString(text));
    }
    void    Remove(int pos, int n);

    void    Insert(const WString& text);
    void    Insert(const String& text) {
        return Insert(text.ToWString());
    }
    void    Insert(const char *text) {
        return Insert(WString(text));
    }
    void    Insert(int chr);
...
public:
    ...
    EditString& MaxLen(int maxlen) {
        ConvertString::MaxLen(maxlen);
        Ctrl::Refresh();
        return *this;
    }
    EditString& NotNull(bool nn = true) {
        ConvertString::NotNull(nn);
        Ctrl::Refresh();
        return *this;
    }
    EditString& TrimLeft(bool b) {
        ConvertString::TrimLeft(b);
        Ctrl::Refresh();
        return *this;
    }
    EditString& TrimRight(bool b) {
        ConvertString::TrimRight(b);
        Ctrl::Refresh();
        return *this;
    }
    EditString& TrimBoth(bool b) {
        ConvertString::TrimBoth(b);
        Ctrl::Refresh();
        return *this;
    }
};
...
public:
    EditDoubleNotNullSpin(double inc = 0.1) : EditDoubleSpin(inc) {
        NotNull();
    }
    EditDoubleNotNullSpin(double min, double max, double inc = 0.1) : EditDoubleSpin(min, max, inc) {
        NotNull();
    }
};
