...
private:
    ...
    void          EnableDrop(bool b = true) {
        MainButton().Enable(b || alwaysdrop);
    }
    void          Sync();
...
public:
    ...
    void          GoBegin() {
        if(GetCount()) SetIndex(0);
    }
    void          GoEnd() {
        if(GetCount()) SetIndex(GetCount() - 1);
    }
    ...
    const PopUpTable& GetList() const {
        return list;
    }
    PopUpTable&   ListObject() {
        return list;
    }
    ...
    DropList&     DropWidthZ(int w) {
        dropwidth = HorzLayoutZoom(w);
        return *this;
    }

    DropList&     SetScrollBarStyle(const ScrollBar::Style& s) {
        list.SetScrollBarStyle(s);
        return *this;
    }
...
public:
    ...
    void            AddHistory(int max = 12) {
        select.AddHistory(this->GetData(), max);
    }

    MultiButton::SubButton& AddButton() {
        return select.AddButton();
    }
    int                     GetButtonCount() const {
        return select.GetButtonCount();
    }
    MultiButton::SubButton& GetButton(int i) {
        return select.GetButton(i);
    }
    Rect                    GetPushScreenRect() const {
        return select.GetPushScreenRect();
    }

    const MultiButton::Style& StyleDefault() {
        return select.StyleFrame();
    }
