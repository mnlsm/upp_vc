...
#include <queue>
#include <functional>
#include "lib/marisa/grimoire/algorithm.h"
#include "lib/marisa/grimoire/trie/header.h"
#include "lib/marisa/grimoire/trie/range.h"
#include "lib/marisa/grimoire/trie/state.h"
#include "lib/marisa/grimoire/trie/louds-trie.h"
...
std::size_t LoudsTrie::io_size() const {
  //size_t a = Header().io_size();
  //size_t b = louds_.io_size();
  //size_t c = terminal_flags_.io_size();
  //size_t d = link_flags_.io_size();
  //size_t e = bases_.io_size();
  //size_t f = extras_.io_size();
  //size_t g = tail_.io_size();
  //size_t h = ((next_trie_.get() != NULL) ?
  //    (next_trie_->io_size() - Header().io_size()) : 0);
  //size_t i = cache_.io_size();
